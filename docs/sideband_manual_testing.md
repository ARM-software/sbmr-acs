...
`- /au
  ...
  <tr>
    <td>5</td>
    <td>Obtain the binding-specific hardware address, then use the MCTP <b>SetupEndpoint</b> method to test MCTP control messaging.<br><pre><code>busctl call &lt;mctp-dbus-service&gt; &lt;bus-owner-object-path&gt; au.com.codeconstruct.MCTP.BusOwner1 SetupEndpoint ay &lt;byte-count&gt; &lt;hwaddr-bytes&gt;</code></pre></td>
    <td>1. The SetupEndpoint call completes without a D-Bus error<br>2. A valid EID, network ID, and endpoint object path are returned. The returned EID and network ID match the corresponding D-Bus properties on the returned endpoint object<br>3. The final Boolean indicates whether a new EID was assigned: <b>true</b> means a new EID was assigned, while <b>false</b> means no new EID assignment was required. Both values indicate success when the other returned values are valid<br>Example command and output (I2C bus: <b>mctpi2c0</b>, hardware address: <b>0x40</b>):<br><pre># busctl call au.com.codeconstruct.MCTP1 /au/com/codeconstruct/mctp1/interfaces/mctpi2c0 au.com.codeconstruct.MCTP.BusOwner1 SetupEndpoint ay 1 0x40<br><br>yisb 18 1 "/au/com/codeconstruct/mctp1/networks/1/endpoints/18" false</pre><br>Note: <code>&lt;hwaddr-bytes&gt;</code> is binding-specific; for example, a 1-byte I2C address or a 6-byte I3C PID. For the <code>ay</code> signature, <code>&lt;byte-count&gt;</code> is the number of hardware-address bytes that follow; it is not the endpoint EID.</td>
  </tr>
  <tr>
    <td>6</td>
