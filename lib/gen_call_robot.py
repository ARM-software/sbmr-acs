...
def init_robot_out_parms(extra_prefix=""):
    ...
    file_prefix = (
        AUTOBOOT_BMC_NICKNAME + "." + extra_prefix + time_string + "."
    )
...
def init_robot_test_base_dir_path():
    ...
    BMCTOOL_DIR_PATH = OBMC_TOOLS_BASE_DIR_PATH + "openbmctool/"
    ...
    gm.set_mod_global(BMCTOOL_DIR_PATH)
    os.environ["BMCTOOL_DIR_PATH"] = str(BMCTOOL_DIR_PATH)
...
def create_robot_cmd_string(robot_file_path, *parms):
    ...
    cmd_buf = create_robot_cmd_string("tools/start_sol_console.robot", BMC_HOST, quiet, test_mode, debug,
    outputdir, output, log, report)

    Would return a string something like this.
    robot -v BMC_HOST:beye6 -v quiet:0 -v test_mode:1 -v debug:1
...
def robot_cmd_fnc(
    ...
    BMCTOOL_DIR_PATH = gm.get_mod_global("BMCTOOL_DIR_PATH")
