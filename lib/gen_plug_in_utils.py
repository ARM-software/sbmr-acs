...
def return_plug_vars(general=True, custom=True, plug_in_package_name=None):
    ...
    - Set a default value for environment variable AUTOBOOT_BMC_NICKNAME/AUTOIPL_FSP1_NICKNAME if it is
      not already set.
    ...
    if os.environ.get("AUTOBOOT_BMC_NICKNAME", "") == "":
        os.environ["AUTOBOOT_BMC_NICKNAME"] = os.environ.get(
            "AUTOBOOT_BMC_HOST", ""
        )
...
def print_plug_in_header():
    ...
    When debug is set, print all plug_prefix variables (e.g. AUTOBOOT_BMC_HOST, etc.) and all plug-in
...
def get_plug_vars(mod_name="__main__", **kwargs):
    ...
    For example, if the environment variable in question is AUTOBOOT_BMC_HOST, this function will set
    global variable AUTOBOOT_BMC_HOST and global variable BMC_HOST.
...
def compose_plug_in_save_dir_path(plug_in_package_name=None):
    ...
    NICKNAME = os.environ.get("AUTOBOOT_BMC_NICKNAME", "")
    if NICKNAME == "":
...
def add_tarball_tools_dir_to_path(quiet=0):
    ...
    AUTOBOOT_BMC_NICKNAME = gm.get_mod_global("AUTOBOOT_BMC_NICKNAME")

    tool_dir_path = (
        AUTOBOOT_BASE_TOOL_DIR_PATH
        + os.environ.get("USER")
        + os.sep
        + AUTOBOOT_BMC_NICKNAME
        + os.sep
    )
