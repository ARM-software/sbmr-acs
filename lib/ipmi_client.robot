...
...             bmc box and execute ipmitool IPMI standard
...
Run External IPMI Standard Command
    ...
    #                               "-vvv"), it will be prepended to this
    #                               command string.
    # fail_on_err                   Fail if the IPMI command execution fails.
    # expected_rc                   The expected return code from the ipmi
    #                               command (e.g. ${0}, ${1}, etc.).
...
Check If IPMI Tool Exist
    ...
    [Arguments]  ${login_host}=${1}

    ${output}=  Run Keyword If  ${login_host} == ${1}
    ...      Execute Command  which ipmitool
    ...  ELSE  Shell Cmd  which ipmitool
...
Create SEL
    ...
    ...  ${IPMI_RAW_CMD['SEL_entry']['Create_SEL'][0]} 0x${sensor_type} 0x${sensor_number}
    ... ${IPMI_RAW_CMD['SEL_entry']['Create_SEL'][1]}
...
Get Current Date from BMC
    ...
    # Split the string and remove first and 2nd last value from the list
    # and join to form %d %b %H:%M:%S %Y date format
