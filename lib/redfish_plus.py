...
class redfish_plus(HttpClient):
    ...
    def rest_request(self, func, *args, **kwargs):
        ...
        response = bmc_redfish.get("/redfish/v1/Managers/${BMC_ID}/EthernetInterfaces", [200, 201])
        ...
        BMC_Redfish.logout
        ${response}=  BMC_Redfish.Get  /redfish/v1/Managers/${BMC_ID}/EthernetInterfaces
        ...
        valid_status_codes = kwargs.pop("valid_status_codes", [200, 201, 202, 204])
    ...
    def patch(self, *args, **kwargs):
        ...
        quiet = 1
        # If Etag available, then add If-Match header in PATCH request
        response = self.rest_request(
            super(redfish_plus, self).get, *args, valid_status_codes=[]
        )
        ...
        if MTLS_ENABLED == "True":
            return self.rest_request(
                self.patch_with_mtls, *args, **kwargs, headers=headers
            )
        else:
            return self.rest_request(
                super(redfish_plus, self).patch, *args, **kwargs, headers=headers
            )
