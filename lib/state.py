...
def get_state(
    ...
    Description of argument(s):
    bmc_host          The DNS name or IP address of the BMC.
                      This defaults to global ${BMC_HOST}.
    bmc_username      The username to be used to login to the BMC.
                      This defaults to global ${BMC_USERNAME}.
    bmc_password      The password to be used to login to the BMC.
                      This defaults to global ${BMC_PASSWORD}.
    os_host           The DNS name or IP address of the operating system.
                      This defaults to global ${OS_HOST}.
    os_username       The username to be used to login to the OS.
                      This defaults to global ${OS_USERNAME}.
    ...
    error_message = gv.valid_value(bmc_host, invalid_values=[None, ""])
    if error_message != "":
        BuiltIn().fail(gp.sprint_error(error_message))

    if bmc_username == "":
        bmc_username = BuiltIn().get_variable_value("${BMC_USERNAME}")
    error_message = gv.valid_value(bmc_username, invalid_values=[None, ""])
    if error_message != "":
        BuiltIn().fail(gp.sprint_error(error_message))

    if bmc_password == "":
        bmc_password = BuiltIn().get_variable_value("${BMC_PASSWORD}")
    error_message = gv.valid_value(bmc_password, invalid_values=[None, ""])
    if error_message != "":
        BuiltIn().fail(gp.sprint_error(error_message))
...
def wait_for_comm_cycle(start_boot_seconds, quiet=None):
    ...
    gp.qprint_timen("Verifying that Redfish API interface is working.")
